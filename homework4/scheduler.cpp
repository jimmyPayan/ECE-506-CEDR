...
int scheduleFSFS(ConfigManager &cedr_config,
                 std::deque<task_nodes *> &ready_queue,
                 worker_thread *hardware_thread_handle,
                 ...
                 uint32_t &free_resource_count)
{
    ...
    if (ready_queue.empty()) {
        std::cout << "[FSFS] Ready queue is empty. Nothing to schedule." << std::endl;
        return 0;
    }

    std::cout << "[FSFS] Starting scheduling for "
              << ready_queue.size() << " task(s)." << std::endl;

    // Get total number of resources (CPU, FFTS, GEMM, ZIP, etc.)
    unsigned int total_resources = cedr_config.getTotalResources();

    // Debug list of all registered resources
    std::cout << "[FSFS] Total registered resources: " << total_resources << std::endl;
    for (unsigned int i = 0; i < total_resources; ++i) {
        std::cout << "[FSFS] Resource[" << i << "] = "
                  << hardware_thread_handle[i].resource_name
                  << " (type=" << hardware_thread_handle[i].thread_resource_type << ")"
                  << std::endl;
    }

    // Iterate through all tasks in ready queue (FIFO order)
    for (auto itr = ready_queue.begin(); itr != ready_queue.end();) {
        task_nodes *task = *itr;

        // Sanity checks
        if (!task) {
            std::cout << "[FSFS] Null task pointer encountered - skipping." << std::endl;
            itr = ready_queue.erase(itr);
            continue;
        }

        if (!task->app_pnt) {
            std::cout << "[FSFS] Null app_pnt for task: " << task->task_name << std::endl;
            itr = ready_queue.erase(itr);
            continue;
        }

        std::cout << "[FSFS] Scheduling task '" << task->task_name
                  << "' (start_time=" << task->app_pnt->start_time << ")" << std::endl;

        bool task_allocated = false;

        // Try assigning this task to *all* available PEs
        for (unsigned int i = 0; i < total_resources; ++i) {
            auto resourceType = hardware_thread_handle[i].thread_resource_type;
            auto resourceName = hardware_thread_handle[i].resource_name;

            // Skip if this resource type can't execute the task
            if (!task->supported_resources[(uint8_t)resourceType]) {
                continue;
            }

            std::cout << "[FSFS] Trying to assign '" << task->task_name
                      << "' to " << resourceName
                      << " (type=" << resourceType << ")" << std::endl;

            // Try to assign the task
            task_allocated = attemptToAssignTaskToPE(
                cedr_config,
                task,
                &hardware_thread_handle[i],
                &resource_mutex[i],
                i
            );

            if (task_allocated) {
                std::cout << "[FSFS] Task '" << task->task_name
                          << "' successfully scheduled on " << resourceName
                          << " (type=" << resourceType << ")" << std::endl;

                itr = ready_queue.erase(itr);
                ++tasks_scheduled;

                // Respect no-queueing mode
                if (!cedr_config.getEnableQueueing()) {
                    free_resource_count--;
                    if (free_resource_count == 0)
                        break;
        ...
        // If not assigned, move to next task
        if (!task_allocated) {
            std::cout << "[FSFS] Could not assign '" << task->task_name
                      << "' to any available PE. Leaving in queue." << std::endl;
            ++itr;
        }
    }

    std::cout << "[FSFS] Scheduling complete. Total tasks scheduled: "
              << tasks_scheduled << std::endl;
