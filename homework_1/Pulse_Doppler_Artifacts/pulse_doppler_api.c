...
void xcorr(double *x, double *y, size_t n_samp, double *corr) {
  ...
  // FFT2 scope
  {
    ...
    //gsl_fft_wrapper(fft_inp, fft_out, len, true /* is_forward_transform? */);
    DASH_FFT_flt(fft_inp, fft_out, len, true);
  ...
  // FFT3 scope
  {
    ...
    //gsl_fft_wrapper(fft_inp, fft_out, len, false /* is_forward_transform? */);
...
int main(int argc, char *argv[]) {
  ...
  for (x = 0; x < 2 * n_samples; x++) {
    ...
    // FFT scope
    {
      ...
      //gsl_fft_wrapper(fft_inp, fft_out, m, true /* is_forward_transform? */);
      DASH_FFT_flt(fft_inp, fft_out, m, true);
