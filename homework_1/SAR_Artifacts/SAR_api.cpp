...
int main(void) {
  ...
  bool forwardTrans = true;
    ...
    using DCFLT = dash_cmplx_flt_type*;

    DASH_FFT_flt((DCFLT) g, (DCFLT) g2, fast, forwardTrans);

  for (i = 0; i < Nslow; i++) {
    DASH_FFT_flt((DCFLT) (s0+i*Nfast), (DCFLT)(fft_out_0+i*Nfast), fast, forwardTrans);

    fftshift((fft_out_0+i*Nfast), Nfast);

    DASH_ZIP_flt((DCFLT)(fft_out_0+i*Nfast), g2, fft_inp_1, Nfast ,ZIP_MULT);
    DASH_FFT_flt((DCFLT) &(fft_inp_1[i*Nfast]), (DCFLT) &(fft_out_1[i*Nfast]), fast, !forwardTrans);
  }
  ...
  for (i = 0; i < Nfast; i++) {
    DASH_FFT_flt((DCFLT) &(fft_inp_3[i*Nslow]), (DCFLT) &(S1[i*Nslow]), slow, forwardTrans);
    fftshift((S1+i*Nslow), Nslow);
  }

  // Azimuth Compression
  for (i = 0; i < Nfast; i++) {
    DASH_ZIP_flt( (DCFLT)(S1+i*Nslow), H, fft_inp_2, Nfast ,ZIP_MULT);
    DASH_FFT_flt((DCFLT) (fft_inp_2+i*Nslow), (DCFLT) (fft_out_2+i*Nslow), slow, !forwardTrans);
  ...
  if (fp != NULL) {
    for (i = 0; i < Nslow; i++) {
      for (j = 0; j < Nfast; j++) {
        fprintf(fp, "%0.10lf ", sac[j + i * Nfast]);
      }
      fprintf(fp, "\n");
      fflush(fp);
    }
